...
5. **Canonical URLs — split by surface (frontend vs marketing).**
   - **`frontend/` (app.betsplug.com):** canonical is always the EN form regardless of the rendering locale. `lib/seo-helpers.ts` → `getCanonicalUrl()` returns `https://betsplug.com{canonicalPath}`; individual pages never build their own canonical. The authed dashboard is internal-search territory — collapsing locales onto one canonical is fine because the surface isn't trying to rank in 16 locales.
   - **`marketing/` (betsplug.com):** canonical is **self-referencing per locale** (e.g. `/de/vorhersagen` → `https://betsplug.com/de/vorhersagen`). Wired in `marketing/src/layouts/BaseLayout.astro` via `getAllLocaleVersions(canonicalPath, SITE)` → pick the entry whose `locale` matches the rendering locale; that becomes both `<link rel="canonical">` and `og:url`. `x-default` keeps pointing at the EN URL. **Do not "fix" this back to EN-form** — marketing needs every locale to rank independently in Google, and EN-canonical-everywhere collapses all 7 locales into one bucket so only EN gets indexed (this was the bug fixed on 2026-05-05). Earlier comments in BaseLayout that said "canonical always points to the EN form per spec" predated the SEO audit and have been removed.
